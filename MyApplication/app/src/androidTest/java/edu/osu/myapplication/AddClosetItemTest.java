...
public class AddClosetItemTest {
    ...
    public void SubType_Pants_isCorrect() {
        onView(withId(R.id.spinner1)).perform(click()); //click on spinner 1
        onData(allOf(is(instanceOf(String.class)), is("Pants"))).perform(click());  // select "Shoes"

        onView(withId(R.id.spinner1)).check(matches(withSpinnerText("Pants")));            //Assert "Shoes is selected
        onView(withId(R.id.spinnerSubType)).check(matches(withSpinnerText("Athletic Shorts")));    //Assert "SubType is "Sandals"
    }
    @Test
    public void SubType_Shirts_isCorrect() {
        onView(withId(R.id.spinner1)).perform(click()); //click on spinner 1
        onData(allOf(is(instanceOf(String.class)), is("Shirt"))).perform(click());  // select "Shoes"

        onView(withId(R.id.spinner1)).check(matches(withSpinnerText("Shirt")));            //Assert "Shoes is selected
        onView(withId(R.id.spinnerSubType)).check(matches(withSpinnerText("None")));    //Assert "SubType is "Sandals"
    }
